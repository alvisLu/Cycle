...
export function HomePage({
  ...
  const currentCycleStartDate = status.currentCycle?.startDate
    ? parseISO(status.currentCycle.startDate)
    : null;
  ...
  const nextPeriod = () => {
    ...
    return (
      <div className={styles.periodBlock}>
        <div className={styles.periodTitle}>下次經期</div>
  ...
  const duringPeriod = () => {
    const message =
      status.daysUntilEnd !== null && status.daysUntilEnd > 0
        ? `剩 ${status.daysUntilEnd} 天`
        : `今天結束`;
    return (
      <div className={styles.periodBlock}>
        <div className={styles.periodTitle}>經期中</div>
        <div className={styles.periodDate}>
          <p className={styles.statusDaysText}>{(status.daysSinceStart ?? 0) + 1} 天</p>
    ...
    <>
      <div className={styles.pageWrapper}>
        ...
        <Card>
          ...
          <CardContent>{status.isOnPeriod ? duringPeriod() : nextPeriod()}</CardContent>
        </Card>
